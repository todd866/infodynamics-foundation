...
\title{A Thermodynamic Foundation for the Second Law of Infodynamics}
...
\noindent\textbf{Keywords:} second law of infodynamics, Landauer principle, symmetry, dimensionality, stochastic thermodynamics, information entropy

\section{Introduction}

The second law of thermodynamics establishes that entropy increases in isolated systems. Landauer's principle \citep{landauer1961, bennett1982} connects this to computation: erasing one bit of information requires dissipating at least $k_{\mathrm B}T\ln 2$ of heat. This has been experimentally verified \citep{berut2012} and forms the foundation of the thermodynamics of information \citep{parrondo2015, seifert2012}.

Recently, Vopson and Lepadatu \citep{vopson2022, vopson2023} proposed a ``second law of infodynamics'' stating that the \emph{information entropy} of physical systems decreases over time. Unlike thermodynamic entropy (which increases), information content in systems such as genetic sequences, digital data, and atomic configurations appears to evolve toward states of lower information entropy---that is, toward simpler, more symmetric configurations. This is consistent with Vopson's earlier mass-energy-information equivalence principle \citep{vopson2019}.

A key observation in Vopson~\cite{vopson2023} is that high symmetry corresponds to low information entropy. Symmetric states require fewer bits to specify; a sphere is described by one parameter (radius), while an irregular shape requires many. Many equilibrium macrostates are highly symmetric, suggesting a connection between symmetry and thermodynamic stability.
...
Following Vopson~\cite{vopson2023}, we observe that symmetric configurations are information-sparse. Consider a probability distribution $p(x)$ over states $x \in \mathbb{R}^D$. A maximally symmetric (isotropic) distribution has the form
\begin{equation}
...
The entropy production in a stochastic process is bounded by the Kullback-Leibler divergence between initial and final distributions \citep{seifert2012}. For a projection process, the dissipated work (work beyond the free energy change) satisfies:
\begin{equation}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{The geometric maintenance bound.} Enforcing a low-dimensional representation $Y = \Phi(X)$ requires work to suppress fluctuations orthogonal to the target manifold. The minimal dissipation separates into an informational term (bits removed) and a geometric contraction term (Jacobian cost).}
...
This connects to Noether's theorem \citep{noether1918} and Weyl's analysis of symmetry \citep{weyl1952}: symmetries reduce the number of independent parameters. A system with $O(D)$ symmetry (full isotropy) has one parameter (scale). A system with broken symmetry has up to $\frac{D(D+1)}{2}$ parameters (full covariance matrix).

From the perspective of Landau theory \citep{landau1937}, the thermodynamic cost of broken symmetry can be understood as the cost of stabilizing the associated Goldstone modes. When rotational symmetry is broken---confining a particle to a ring in 2D, for example---the radial degree of freedom must be suppressed. Thermal fluctuations continuously excite this ``soft mode,'' and the controller must pay the housekeeping cost to maintain the low-dimensional manifold. The more symmetries are broken, the more Goldstone-like modes must be stabilized, and the higher the maintenance power \citep{anderson1972}.

This perspective also illuminates why biological measurement systems face fundamental limits in the sub-Landauer domain \citep{todd2025falsifiability}: the work required to maintain an ordered (asymmetric) state places a floor on the energetic cost of biological information processing. The ``projection bound'' governing Maxwell's demon in continuous substrates \citep{todd2025maxwell} reflects the same geometric maintenance cost applied to measurement.
...
Living systems are paradigmatic examples of maintained asymmetry. Cells confine molecular distributions to specific compartments; neural systems maintain low-dimensional activity manifolds \citep{cunningham2014, gallego2017}; organisms maintain thermal and chemical gradients far from equilibrium.
